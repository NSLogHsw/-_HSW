...
#import "SWGameBaseViewController.h"
//定义标识
static NSString *const ID = @"cell";
@interface SWGameBaseViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation SWGameBaseViewController
...
- (void)setupTopTitleView
{
    ...
    UIScrollView *topView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 64, SWScreenWidth, 35)];
...
- (void)setupBottomContentView
{
    ...
    flowLayout.itemSize = CGSizeMake(SWScreenWidth, SWScreenHeight);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.view.frame = CGRectMake(0, 0, SWScreenWidth,SWScreenHeight);
...
{
    NSInteger i = scrollView.contentOffset.x / SWScreenWidth;
...
- (void)setupAllTitle
{
    ...
    CGFloat btnW = SWScreenWidth / _topTitleBtn;
    CGFloat btnX;
    CGFloat btnH = _topView.gf_height;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (i == 0) {
            CGFloat h = 2;
            CGFloat y = titleBtn.gf_height - h;
            UIView *underLine = [[UIView alloc]init];
            underLine.gf_centerX = titleBtn.gf_centerX ;
            underLine.gf_x = 20;
            underLine.gf_height = h;
            [titleBtn.titleLabel sizeToFit];
            underLine.gf_width = titleBtn.titleLabel.gf_width * 1.3;
            underLine.gf_y = y;
...
- (void)selButton:(UIButton *)titleBtn
{
    ...
    CGFloat offsetX = titleBtn.center.x - SWScreenWidth * 0.5;
    
    if (offsetX < 0) {
        offsetX = 0;
    }
    
    CGFloat MaxOffserX = self.topView.contentSize.width - SWScreenWidth;
    ...
    [UIView animateWithDuration:0.25 animations:^{
        _underLine.gf_centerX = titleBtn.gf_centerX;
    }];
...
- (void)titleClick:(UIButton *)titleBtn
{
    ...
    CGFloat offsetX = i * SWScreenWidth;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    //设置前一个按钮的索引
    NSInteger leftI = scrollView.contentOffset.x / SWScreenWidth;
    //设置后一个按钮的索引
    ...
    //设置按钮文字的大小缩放
    CGFloat scaleR = scrollView.contentOffset.x / SWScreenWidth - leftI;
...
//    _underLine.gf_x = scrollView.contentOffset.x / _titleBtns.count ;
//    _underLine.gf_x= scrollView.gf_x / _titleBtns.count +1;
